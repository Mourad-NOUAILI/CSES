 ...
 */
void range_update_lazy(ll* & seg, ll* & lazy, ll start_pos, ll end_pos, ll pos, ll l, ll r, ll val, ll op){
    if (lazy[pos] != 0){
    ...
    if(l <= start_pos && r >= end_pos){
        int a = val;
        if (op == 2) a = val-seg[pos];
        seg[pos] += (end_pos-start_pos+1)*a;
        if (start_pos != end_pos){
            int b = val;
            if (op == 2) b = val-lazy[2*pos+1];
            lazy[2*pos+1] += b;
            int c = val;
            if (op == 2) c = val-lazy[2*pos+2];
            lazy[2*pos+2] += c;
        }
        return;
    }
    ...
    ll mid = (start_pos+end_pos) >> 1;
    range_update_lazy(seg,lazy,start_pos,mid,2*pos+1,l,r,val,op);
    range_update_lazy(seg,lazy,mid+1,end_pos,2*pos+2,l,r,val,op);
...
int main() {
    ...
    for (ll i = 1 ; i <= q ; ++i){
        ...
        if (op == 1 || op == 2){
            ll l,r,v;
            std::cin >> l >> r >> v;
            range_update_lazy(seg,lazy,0,n-1,0,--l,--r,v,op);
            /*for (ll i = 0 ; i < m ; ++i) std::cout << seg[i] << ' ';
            std::cout << '\n';
